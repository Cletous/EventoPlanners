...
export default function UserEventDetails({ event, onClose, onRegistered }) {
  ...
  const availablePlaces = Math.max(
    Number(event.capacity) - registrationCount,
    0,
  );
      ...
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-slate-500">
                <CircleDollarSign size={16} /> Registration fee
              </div>
              <p className="mt-2 font-bold text-slate-900">
                {Number(event.registration_fee) === 0
                  ? "Free"
                  : `US$${Number(event.registration_fee).toFixed(2)}`}
              </p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-slate-500">
              ...
              <p className="mt-2 font-bold text-slate-900">
                {availablePlaces} of {event.capacity} places available
              </p>
            </div>
          </div>

          {message && (
            <div className="mt-6 rounded-2xl border border-green-100 bg-green-50 p-4 text-sm font-semibold text-green-700">
              {message}
            </div>
          )}
          {error && (
          ...
          {Number(event.registration_fee) > 0 && !message && !isFull && (
            <p className="mt-3 text-center text-sm text-slate-500">
              This is a paid event. Your registration will remain pending until
